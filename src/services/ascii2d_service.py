...
class Ascii2DService:
    ...
    def __init__(self):
        ...
        logger.info(f"Ascii2D服务初始化完成 - 基础URL: {self.base_url}, BOVW: {self.bovw}")
        ...
        try:
            ...
            async with Network(verify_ssl=True) as client:
                ...
                if response and hasattr(response, 'raw') and response.raw:
                    ...
                    # 转换搜索结果
                    results = self._convert_search_results_from_raw(response.raw, "image_url")
        ...
        try:
            ...
            async with Network(verify_ssl=True) as client:
                ...
                if response and hasattr(response, 'raw') and response.raw:
                    logger.info(f"搜索成功，找到 {len(response.raw)} 个结果")
                    # 转换搜索结果
                    results = self._convert_search_results_from_raw(response.raw, "uploaded_image")
    ...
    def _convert_search_results_from_raw(self, raw_results: list, source_type: str) -> List[ImageSearchResult]:
        """直接从原始搜索结果列表转换"""
        results = []
    ...
    async def search_multiple_engines(self, image_data: bytes, engines: List[str] = None) -> Dict[str, List[ImageSearchResult]]:
        """使用多个搜索引擎进行搜索"""
        if engines is None:
            engines = ["ascii2d"]
        ...
        try:
            for engine in engines:
                if engine.lower() == "ascii2d":
                    ...
                    logger.info(f"Ascii2D引擎搜索完成，找到 {len(engine_results)} 个结果")
                # 这里可以扩展其他搜索引擎
