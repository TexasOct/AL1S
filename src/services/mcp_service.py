...
class MCPService:
                    ...
                    async with stdio_client(server_params) as (read, write):
                        async with ClientSession(read, write) as session:
                            ...
                            try:
                                ...
                                logger.info(f"MCP服务器 {server_config.name} 连接成功: {server_info}")
                            except AttributeError:
                                logger.info(f"MCP服务器 {server_config.name} 连接成功（服务器信息不可用）")
                            ...
                            if tools_result.tools:
                                ...
                                for tool in tools_result.tools:
                                    ...
                                    logger.info(f"发现工具: {tool.name} - {tool.description}")
                            else:
                                logger.warning(f"MCP服务器 {server_config.name} 未提供任何工具")
    ...
    async def call_tool(self, tool_name: str, arguments: Dict[str, Any], server_name: str = None) -> Optional[str]:
        """调用MCP工具"""
        if not MCP_AVAILABLE:
