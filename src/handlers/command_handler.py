...
class CommandHandler(BaseHandler):
    ...
    async def _execute_command(self, cmd: Command, args: str, update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
        """执行命令"""
        try:
            if cmd.name == "start":
    ...
    async def _handle_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
        """处理start命令"""
        try:
            user_id = update.effective_user.id
...
• 使用 /search 命令搜索网络图片
    ...
    async def _handle_role(self, update: Update, context: ContextTypes.DEFAULT_TYPE, args: str) -> bool:
        """处理角色命令"""
...
告别语: {current_role.farewell}
    ...
    async def _handle_roles(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
        """处理角色列表命令"""
    ...
    async def _handle_create_role(self, update: Update, context: ContextTypes.DEFAULT_TYPE, args: str) -> bool:
        """处理创建角色命令"""
        if not args:
            await update.message.reply_text("❌ 请提供角色名称和描述\n用法: /create_role 名称 描述")
            return False
        ...
        # 使用OpenAI生成角色提示词
        system_prompt = await self.openai_service.generate_role_prompt(description)
        
        if self.conversation_service.create_custom_role(name, description, system_prompt):
            await update.message.reply_text(f"✅ 角色 {name} 创建成功！\n使用 /role {name} 来切换到该角色")
        else:
...
当前角色: {stats.get('current_role', 'N/A')}
    ...
    async def _handle_ping(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
        """处理ping命令"""
        await update.message.reply_text("🏓 Pong! 机器人运行正常")
        return True
    
    async def _handle_search(self, update: Update, context: ContextTypes.DEFAULT_TYPE, args: str) -> bool:
        """处理图片搜索命令"""
        if not args:
    ...
    async def _handle_search_engines(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
        """处理搜索引擎信息命令"""
        engines_text = """
🔍 可用的图片搜索引擎：
...
• 支持多种图片格式
    ...
    async def _handle_test_search(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
        """处理测试搜索命令"""
        test_text = """
🧪 图片搜索服务测试
...
开始和我聊天吧！您可以问我任何问题，或者发送图片让我分析。"""
            ...
            await self._send_formatted_response(update, welcome_text, "欢迎使用 AL1S-Bot")
        ...
        try:
            ...
            for i, role_name in enumerate(roles, 1):
                ...
                if role_config:
                    ...
                    if hasattr(role_config, 'english_name') and role_config.english_name:
        ...
        try:
            ...
            if not args:
                ...
                if current_role:
                    ...
                    if hasattr(current_role, 'english_name') and current_role.english_name:
                        role_text += f"<b>英文名:</b> {current_role.english_name}\n"
                    if hasattr(current_role, 'description') and current_role.description:
                        role_text += f"<b>描述:</b> {current_role.description}\n"
                    if hasattr(current_role, 'personality') and current_role.personality:
                    ...
                    await self._send_formatted_response(update, role_text, "当前角色信息")
                else:
            ...
            if success:
                ...
                await update.message.reply_text(f"❌ 切换到角色 '{role_name}' 失败，请检查角色名称是否正确")
